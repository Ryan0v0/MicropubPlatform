...
def export_micropubs(*args, **kwargs):
    ...
    try:
        ...
        text_body = '''
        Dear {},
        Please find attached the archive of your micropubs that you requested.
        Sincerely,
        Team MP-AM-2
        Note: replies to this email address are not monitored.
        ...
        html_body = '''
        <p>Dear {0},</p>
        <p>Please find attached the archive of your micropubs that you requested.</p>
        <p>Sincerely,</p>
        <p>Team MP-AM-2</p>
        <p><small>Note: replies to this email address are not monitored.</small></p>
        '''.format(user.username)
        '''
        send_email('[MicroPub] 导出微知识',
                   sender=app.config['MAIL_SENDER'],
                   recipients=[user.email],
                   text_body=text_body,
                   html_body=html_body,
                   attachments=[('micropubs.json', 'application/json',
                                 json.dumps({'micropubs': data}, indent=4))],
                   sync=True)
        '''
    except Exception:
        _set_task_progress(100)
        app.logger.error('[导出微知识]后台任务出错了', exc_info=sys.exc_info())
